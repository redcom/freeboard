...
function FreeboardModel(datasourcePlugins, widgetPlugins, freeboardUI)
{
	...
	this.setEditing = function(editing, animate)
	{
		...
		if(!editing)
		{
			...
			$("#hamburger").animate({"top": "-" + (barHeight - hamburgerTop) + "px"}, animateLength);
			$("#main-header").animate({"top": "-" + barHeight + "px"}, animateLength);
			$("#board-content").animate({"top": "20px"}, animateLength);
	...
	this.setVisibilityBoardTools = function(visibility, animate)
	{
		...
		if (visibility == true) {
			...
			barElem.animate({'left' : 0 + "px" }, animateLength);
			$("#hamburger").animate({"left": barWidth + "px"}, animateLength);
			$("#main-header").animate({"left": barWidth + "px"}, animateLength);
			$("#board-content").animate({"left": barWidth + "px"}, animateLength);
			$(window).resize(fnResizer);
		} else {
			$("html").removeClass("boardtools-opening");
			$("#board-actions > ul").addClass("collapse");
			barElem.animate({'left' : "-" + barWidth + "px" }, animateLength);
			$("#hamburger").animate({"left": "0px"}, animateLength);
			$("#main-header").animate({"left": "0px"}, animateLength);
			$("#board-content").animate({"left": "0px"}, animateLength);
			$(window).off('resize', fnResizer);
		}
	}
