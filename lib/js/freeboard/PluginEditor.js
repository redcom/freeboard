...
PluginEditor = function(jsEditor, valueEditor) {
    ...
    function createPluginEditor(title, pluginTypes, currentTypeName, currentSettingsValues, settingsSavedCallback) {
        ...
        new DialogBox(form, title, $.i18n.t('PanelEditor.dialog.yes'), $.i18n.t('PanelEditor.dialog.no'), function(okcancel) {
            if (okcancel == "ok") {
        ...
        if (pluginTypeNames.length > 1) {
            var typeRow = createSettingRow("plugin-types", $.i18n.t('PanelEditor.type'));
            typeSelect = $('<select></select>').appendTo($('<div class="styled-select"></div>').appendTo(typeRow));

            typeSelect.append($("<option>"+$.i18n.t('PanelEditor.firstOption')+"</option>").attr("value", "undefined"));
